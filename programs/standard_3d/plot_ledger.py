...
plt.figure()
plt.title('N2 excitations relative')
plt.stackplot(timesteps, np.transpose(N2_exc_relative), labels=np.arange(2,24).astype(str))
plt.legend()

plt.figure()
plt.title('N2 excitations total')
plt.stackplot(timesteps, np.transpose(ledger[:, 1:23]), labels=np.arange(2,24).astype(str))
plt.legend()

plt.figure()
plt.title('N2 excitations relative')
plt.semilogy(timesteps, N2_exc_relative)#, labels=np.arange(2,24).astype(str))
plt.semilogy(timesteps, ledger[:, 1:23])#, labels=np.arange(2,24).astype(str))
plt.legend()
